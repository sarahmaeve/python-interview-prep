# Your Exercise Times

Copy this file to `YOUR_TIMES.md` (gitignored) and fill in your actual times
as you work through the exercises. Comparing to the estimates in
`TIME_ESTIMATES.md` tells you where you're strong and where to study more.

| # | Exercise | Estimated | Your Time | Delta | Notes |
|---|----------|-----------|-----------|-------|-------|
| 01 | Shopping Cart            | 10 min | | | |
| 02 | Text Formatter           | 15 min | | | |
| 03 | Bank Account             | 15 min | | | |
| 04 | Shapes                   | 15 min | | | |
| 05 | User Validator           | 20 min | | | |
| 06 | Event Logger             | 20 min | | | |
| 07 | Config Parser            | 20 min | | | |
| 08 | Task Manager             | 20 min | | | |
| 09 | Weather Client           | 25 min | | | |
| 10 | Cache with Expiry        | 25 min | | | |
| 11 | CSV Report               | 25 min | | | |
| 12 | Notification Service     | 30 min | | | |
| 13 | Grade Processor          | 15 min | | | |
| 14 | Task Registry            | 10 min | | | |
| 15 | Payment Processor        | 15 min | | | |
| 16 | Inventory Service        | 15 min | | | |
| 17 | Temperature Monitor      | 15 min | | | |
| 18 | String Calculator        | 10 min | | | |
| 19 | Order Service            | 20 min | | | |
| 20 | Black Box Wrapper        | 20 min | | | |
| 21 | Observability & Logging  | 25 min | | | |
| 22 | Systems Integration      | 25 min | | | |
| 23 | Dataclass Refactor       | 20 min | | | |
| 24 | Money and Decimal        | 15 min | | | |
| 25 | Enum State Machine       | 15 min | | | |
| 26 | pathlib Bugs             | 20 min | | | |
| 27 | match/case Dispatch      | 15 min | | | |
| 28 | Context Manager Leaks    | 20 min | | | |
| 29 | Async Retry              | 25 min | | | |
| 30 | Pytest Translation       | 20 min | | | |
| 31 | Decorator @wraps         | 15 min | | | |
| 32 | Thread Safety            | 25 min | | | |
| 33 | Capstone: Order Pipeline | 45 min | | | |
| 34 | Flaky Tests              | 25 min | | | |
| 35 | Debugging with pdb       | 25 min | | | |
| | **Total** | **~660 min (~11 hrs)** | | | |
